...
public class UserController {
    ...
    // Check if password provided by the user satisfies the minimum requirements
    // Returns boolean
    private Boolean isPasswordValid(String password) {
        // Initializing the regex pattern
        Pattern pattern = Pattern.compile("(?=.*[a-z])(?=.*[0-9])(?=.*[^a-z0-9])", Pattern.CASE_INSENSITIVE);
        // Matching the above pattern with the password provided
        Matcher match = pattern.matcher(password);
        // Returns true or false if password matches the pattern or not
        return match.find();
    }
}
