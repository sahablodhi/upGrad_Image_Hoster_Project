...
// mentioned as 'tags'. Hence the table named 'tags' will be created in the
// database with all the columns mapped to all the attributes in 'Tag' class
@Table(name = "tags")
public class Tag {
    ...
    // Write the annotation for many to many between images and tags where they are
    // mapped by tags field in the images table
    // The 'tags' table is mapped to 'images' table with Many:Many mapping
    // One image can have multiple categories/tags and there can be multiple images
    // under one category/tag
    // FetchType is LAZY
